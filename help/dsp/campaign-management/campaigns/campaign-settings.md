...
* **[!UICONTROL Composite Margin %]:** (Campaigns that use [!UICONTROL Margin % of Total Budget] with composite margins) The percentage of the gross spend that, to be withheld as adobe tech fees and agency fees combined. Agency fees is computed by substracting the Adobe tech fees from the composite margin amount. Any changes to the composite margin value are applied to future gross spend only and not to the historical gross spend for the campaign. The [!UICONTROL Estimated Tax Withholding] value is excluded from the gross spend before the composite margin is applied.
...
* **[!UICONTROL Markup %]:** (Campaigns that use [!UICONTROL Apply Markup % on top of individual cost components]) The percentage to apply to specified cost components to compute agency fees. Any changes to the markup value are applied to future future costs only and not to the historical costs for the campaign.
