...
**[!UICONTROL DoubleVerify Account]:** ([!DNL DoubleVerify] customers only; optional) A [!DNL DoubleVerify Authentic Brand Suitability] segment ID associated with the organization's [!DNL DoubleVerify] account to use by default for all placements. Specifying an ID blocks impressions post-bid using the custom brand suitability rules configured for the specified segment ID. DSP bills your account for usage for the segment ID.
