...
* **EF Errors:** When the bulksheet operation can't upload or process a file or individual rows in the file, it creates an error file called `<uploaded file name>_ef_errors.<extension used for the bulksheet>`. If the problem is with individual rows, then those rows are included, with an explanation of each error so you can correct it. The errors are included in the "[!UICONTROL EF Error Message]" column.
