---
title: (New UI) About the [!UICONTROL Ads] view
description: Learn what you can do in the [!UICONTROL Ads] view.
feature: Search Optimization, Search Campaign Management
hide: true
exl-id: 6a0a4b74-a767-4c4d-a823-0fbc3c738cd0
product_v2:
  - id: a829a185-511f-4bf8-8dcf-9e684f8011cf
    internal-label: Advertising
feature_v2:
  - id: aed5e38a-3e62-42fa-8d16-cd080729b2a0
    internal-label: Search, Social, & Commerce
subfeature_v2:
  - id: f3d33161-c519-436e-bbbd-730ba428736b
    internal-label: Campaign management
---
# (New UI) About the [!UICONTROL Ads] view

*Beta feature*

The [!UICONTROL Manage] > [!UICONTROL Ads] view lists all ads in the filtered view for the selected advertiser account.


## Available actions

* [Manage data view reports from the [!UICONTROL Ads] view](ad-view-report.md)

* [Change the status of an ad](ad-change-status.md)
<!--
* [Assign constraints to ad groups, and unassign constraints from ads](/help/search-social-commerce/new-ui/manage/ads/ad-constraint-assignments-manage.md)
-->

<!--  >* [Manage constraint assignments for ads](/help/search-social-commerce/new-ui/manage/ads/ad-constraint-assignments-manage.md)  -->

>[!MORELIKETHIS]
>
>* [(New UI) Manage data view reports from the [!UICONTROL Ads] view](ad-view-report.md)
>* [(New UI) Change the status of an ad](ad-change-status.md)
