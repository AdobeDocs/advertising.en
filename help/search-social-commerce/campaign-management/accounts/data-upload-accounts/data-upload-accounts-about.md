...
If you use online ad networks for which Search, Social, & Commerce doesn't provide API support, you can upload campaign content and cost, click, and conversion data for an account for reporting and performance simulations<!-- But not optimization? Spend recommendations only if it's in a portfolio?  Sync wording with "Features" section below. -->, but without automated bidding or optimization, to streamline cross-channel insights and identify opportunities for <!-- manual -->optimization. This feature is available for ad networks that follow the standard three-tiered campaign > ad group or ad set > bid unit (ad or keyword) structure; for Adobe DSP, the feature is available for packages and the placements assigned to a package, but not for campaigns nor placements with placement-level pacing.
