...
* `<ID5_PartnerID>` is the organization's ID5 partner ID, which the organization receives after signing an agreement with [!DNL ID5]. Include this variable only when the organization uses DSP and has [custom segments that track users associated with ID5 universal IDs](/help/dsp/audiences/universal-ids.md).
